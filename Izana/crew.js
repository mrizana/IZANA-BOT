...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '242066840860@s.whatsapp.net'; // ton numéro ou le groupe cible
                const imagePath = './database/IzanaCo.jpg';
                ...
                const messageText = `
╔══════════════════╗
      *Izana Bot Connected Successfully* 🚀
╠══════════════════╣
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 242066840860; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '242066840860' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'LORDIZANA');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['242066840860@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
